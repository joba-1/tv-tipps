...
- **Scoring prompt shape**: candidate text is `short_desc + long_desc`, whitespace-collapsed and capped at `_CAND_DESC_MAX_CHARS = 600` (the tail beyond that is the credits block — though for the ~10 % of `long_desc` values that carry one, the `Regie: …` line starts before the cap and survives). The user prefix carries up to `_REACTION_LIMIT = 200` likes/dislikes and `_HISTORY_LIMIT = 70` sessions; both were raised from 52/40 on 2026-09-18 because the old caps silently dropped the longest-standing signals. Whole batch ≈ 19k prompt + ~4.5k output tokens, ~38–45 s.
